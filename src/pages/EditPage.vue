  ...
  <div class="relative flex min-h-screen font-montserrat isolate">
    <div class="mx-auto w-full max-w-[1440px] grid-cols-12 gap-[24px] px-4 xl:px-[120px] my-32">
      <section class="flex justify-between items-center py-4 lg:pb-[7.5rem]">
        <h1 class="text-heading-2 font-bold text-white mb-8">Update movie {{ id }}</h1>
      </section>
        ...
        <div>
          <div
            ...
            <template v-if="!imagePreview">
              <svg class="mb-4" width="24" height="24" viewBox="0 0 24 24" fill="none"
            ...
            <cropper v-else :src="imagePreview" :canvas="false" :resizeImage="{ wheel: false }" :stencil-size="{
              width: 750,
              height: 1000
            }" :stencil-props="{
              handlers: {},
            ...
            <div class="flex absolute p-4 top-0 right-0 items-center justify-start gap-x-2 ">
              <IButton type="button" outline v-if="imagePreview" size="sm" @click="resetImage">
          ...
          <div v-if="errors?.thumbnail" class="text-error mt-1 text-caption">{{ errors?.thumbnail ? errors.thumbnail[0]
            : null }}
          </div>
        </div>
...
<script setup>
defineOptions({
  name: "EditPage", // Change to appropriate name
});
...
const resetImage = () => {
  ...
  movieForm.value.thumbnail = null;
};
...
const submit = async () => {
  ...
  try {
    await movieStore.updateMovie(movieForm.value.id, movieForm.value);
...
onMounted(async () => {
  ...
  if (movieData) {
    movieForm.value.id = movieData.id;
